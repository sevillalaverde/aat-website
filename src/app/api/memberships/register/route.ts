...
export async function POST(req: Request) {
  try {
    ...
    const { name, symbol, chain, contract, website, twitter, telegram, description, contact } = body || {};

    if (!name || !symbol || !chain || !contract) {
      console.warn("register: missing fields", { name, symbol, chain, contract });
    ...
    const exists = tokens.find(
      (t) =>
        t.id === id ||
        (t.chain.toLowerCase() === String(chain).toLowerCase() &&
          t.contract.toLowerCase() === contractNorm.toLowerCase())
    );
    if (exists) {
      console.info("register: already listed", { id, chain, contract: contractNorm });
      return NextResponse.json({ ok: false, error: "already_listed" }, { status: 409 });
    }
    ...
    console.log("token-registered:", { TOKENS_PATH, count: tokens.length, id: token.id, symbol: token.symbol });
