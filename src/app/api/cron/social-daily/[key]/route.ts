...
// helper: interpret env flags like "1/true/yes/on"
const enabled = (v?: string, d = false) => {
...
export async function GET(_req: Request, ctx: { params: { key: string } }) {
  ...
  const links = (process.env.SOCIAL_LINKS || "").split(/\s+/).filter(Boolean);
  const custom = topic === "custom" ? (process.env.SOCIAL_CUSTOM || "") : undefined;

  // Which channels (default OFF for safety)
  const channels = {
    x: enabled(process.env.SOCIAL_CHANNEL_X, false),
    telegram: enabled(process.env.SOCIAL_CHANNEL_TELEGRAM, false),
    discord: enabled(process.env.SOCIAL_CHANNEL_DISCORD, false),
  };

  // Reuse preview generator to build final copy
  ...
  if (!preJson?.ok) return NextResponse.json({ ok: false, error: "preview_failed" }, { status: 500 });
